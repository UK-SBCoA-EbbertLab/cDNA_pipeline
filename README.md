...
## Pipeline parameters for STEP 2 (QC, Alignment, and Bambu Pre-processing)
          ...
          --trim_dRNA                   <Logical, set to "True" if you want to trim dRNA adapters using porechop_ABI.
            ...
            --qscore_thresh               <Minimum mean base quality from basecalled sequence. For example, when set at 9 all reads with 
                                          mean base quality below 9 will be filtered out from the fastq file. IMPORTANT NOTE: There is a good chance
                                          that the fastq file you are using only contains "pass" reads, which are usually Qscore >= 9, therefore filtering by
                                          a Qscore < 9 will still result in only having reads with Qscore >= 9. If you want to filter by a lower Qscore make sure
                                          your fastq file has not been previously filtered. Default: 9>
