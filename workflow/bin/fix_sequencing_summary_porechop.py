...
df_txt = df_txt[["read_id", "run_id", "channel", "start_time", "mean_qscore_template", "sequence_length_template"]].copy()
...
df_txt[["channel", "start_time", "mean_qscore_template", "sequence_length_template"]] = df_txt[["channel", "start_time", 
    "mean_qscore_template", "sequence_length_template"]].apply(pd.to_numeric, errors="ignore").copy()
df_txt.dropna(inplace=True, axis=0)

## Create dataframe with pychopper info
df_read_converter = pd.DataFrame()
df_read_converter["read_id"] = np.asarray(read_ids_list)
df_read_converter["sequence_length_template_porechop"] = np.asarray(read_length_list).astype('int32')
